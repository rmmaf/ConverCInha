...
public class FileReceiverJava extends Thread {
	...
	public FileReceiverJava(ALTCPSocket receberDados, GUI gui, byte oedo) {
		this.receberDados = receberDados;
		if (oedo == 1) {
			this.local = gui.local1.getText();
		} else if (oedo == 2) {
			this.local = gui.local2.getText();
		} else if (oedo == 3) {
			this.local = gui.local3.getText();
		}
	}
